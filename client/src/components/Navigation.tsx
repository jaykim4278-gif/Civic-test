...
export function Navigation() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-border shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] md:static md:border-none md:shadow-none md:bg-transparent">
      <div className="max-w-md mx-auto md:max-w-4xl px-4 h-16 md:h-20 flex items-center justify-center md:justify-start md:gap-8">
        {/* Desktop Logo */}
        ...
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link key={href} href={href} className={cn(
              "flex flex-col md:flex-row items-center justify-center gap-1 md:gap-3 px-6 py-2 rounded-xl transition-all duration-200",
              isActive 
